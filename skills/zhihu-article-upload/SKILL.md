---
name: zhihu-article-upload
description: 当 agent 需要完成知乎登录、cookie 校验、自动导入 Markdown/GitHub/网页内容并发布知乎文章（默认保存草稿）时使用。使用本仓库 TypeScript 引擎与 Playwright；命令为 node dist/cli.js zhihu check|login|publish（或 npm run dev --）。
---
...
4. 仅在扫码、验证码或显式人工确认时暂停并请求用户操作；完成后由 Agent 继续后续步骤。
...
```bash
node dist/cli.js zhihu check --account <name>
node dist/cli.js zhihu login --account <name>
...
| 功能 | CLI |
| --- | --- |
| cookie 校验 | `zhihu check` |
| 登录 / 刷新 storageState | `zhihu login` |
| 发布文章（默认草稿） | `zhihu publish` |

## cookie 持久化

- 默认目录：`$SOCIAL_PUBLISH_DATA_DIR/cookies/zhihu/<account>.json`
- 发布前后引擎会读写 Playwright `storageState`

## 平台边界

- 支持知乎文章，不支持知乎想法、回答、视频或专栏高级设置。
- 知乎编辑器 DOM 变化较频繁；失败时先用 `SOCIAL_PUBLISH_HEADLESS=0` 重试，并按错误里的选择器诊断判断入口或编辑器是否变化。
