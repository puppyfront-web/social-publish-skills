---
name: baijiahao-article-upload
description: 当 agent 需要完成百家号登录、cookie 校验、自动导入 Markdown/GitHub/网页内容并发布百家号文章（默认保存草稿）时使用。使用本仓库 TypeScript 引擎与 Playwright；命令为 node dist/cli.js baijiahao check|login|publish（或 npm run dev --）。
---
...
```bash
node dist/cli.js baijiahao check --account <name>
node dist/cli.js baijiahao login --account <name>
...
| 功能 | CLI |
| --- | --- |
| cookie 校验 | `baijiahao check` |
| 登录 / 刷新 storageState | `baijiahao login` |
| 发布文章（默认草稿） | `baijiahao publish` |

## cookie 持久化

- 默认目录：`$SOCIAL_PUBLISH_DATA_DIR/cookies/baijiahao/<account>.json`
