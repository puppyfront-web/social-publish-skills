...
description: 当 agent 需要按 JSON 配置顺序执行多平台发布、统一 cookie/进度阶段、并兼容 Linux/macOS/Windows 时使用。本仓库引擎为 TypeScript + Playwright，已实现 tencent / douyin / kuaishou / xiaohongshu / wechatmp / zhihu / baijiahao 七个平台。
---
...
- `src/platforms/xiaohongshu.ts`：小红书创作者平台
- `src/platforms/wechatmp.ts`：微信公众号图文
- `src/platforms/zhihu.ts`：知乎文章
- `src/platforms/baijiahao.ts`：百家号文章
...
| 小红书 | `xiaohongshu` | `cookies/xiaohongshu/<account>.json` | 已实现 |
| 微信公众号图文 | `wechatmp` | `cookies/wechatmp/<account>.json` | 已实现 |
| 知乎文章 | `zhihu` | `cookies/zhihu/<account>.json` | 已实现 |
| 百家号文章 | `baijiahao` | `cookies/baijiahao/<account>.json` | 已实现 |
