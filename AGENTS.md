...
3. **路径**：`--file` 使用用户提供的**绝对路径**；JSON 配置里的 `video_file` 也使用绝对路径。
...
| 多平台顺序发 | 复制并编辑 `skills/multi-platform-publish-orchestrator/references/orchestrator.config.example.json`，再 `node dist/cli.js orchestrate --config <该文件路径>` |
